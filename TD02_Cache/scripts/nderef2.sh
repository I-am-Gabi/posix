...
# $Id: nderef2.sh,v 1.2 2008/02/29 16:21:12 jpr Exp $
#------------------------------------------------------------------
# Script de test de l'effet de la période de déréférençage dans 
# la stratéset xlabel "Période de déréférençage (nombre d'accès)"set xlabel "Période de déréférençage (nombre d'accès)"gie NUR en fonction de la taille du cache : 
# zoom sur les petites valeurs de nderef
#------------------------------------------------------------------

file=$1; shift
rm -f $file.gp $file

for nd in 5 10 20 50 100 150 200 250 300 350 400 500
do
    /bin/echo -n $nd
...
set output "$file.eps"
plot "$file" using 1:2 t "r=1000", "$file" using 1:3 t "r=500", "$file" using 1:4 t "r=300", "$file" using 1:5 t "r=200", "$file" using 1:6 t "r=100", "$file" using 1:7 t "r=50", "$file" using 1:8 t "r=20", "$file" using 1:9 t "r=10"
EOF
