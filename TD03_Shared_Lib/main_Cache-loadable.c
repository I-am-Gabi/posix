 ...
 * $Id: main_Cache-loadable.c,v 1.2 2008/04/14 15:15:49 jpr Exp $
 * ------------------------------------------------------------------------------------
 * Programme de test avec chargement dynamique de la stratégie
 * ------------------------------------------------------------------------------------
 */
...
/* ------------------------------------------------------------------------------------
 * Déclaration de la fonction de chargement de la stratégie
 * ------------------------------------------------------------------------------------
*/

extern int Load_Strategy(const char *strategy_name);
...
#define NTESTS 5
static int Do_Test[NTESTS] = {0, 0, 0, 0, 0};
static void (*Tests[NTESTS])() = {
    Test_1,
    Test_2,
    Test_3,
    Test_4,
 ...
 * Création et initalisation le cache, 
 * Exécution des tests
...
int main(int argc, char *argv[])
{
    ...
    if ((The_Cache = Cache_Create(File, N_Blocks_in_Cache, N_Records_per_Block,
                                  sizeof(struct Any), N_Deref)) == NULL)
	Error("Cache_Init");
    Print_Parameters();
...
static void Scan_Args(int argc, char *argv[])
{
	...
	{
            /* Le seul paramètre qui ne soit pas une option est le nom de la stratégie */
	    Load_Strategy(argv[i]);
            strategy_loaded = 1;
	}
    }

    if (!strategy_loaded) Load_Strategy("RAND");
    ...
    if (ntests == 0)
    {
        for (i = 0; i < NTESTS; ++i) Do_Test[i] = 1;
    }
}
